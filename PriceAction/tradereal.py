...
while True:
    print(f"Saldo: {balance:.2f}, Posições: {positions:.4f}, Preço: {df.iloc[-1]['close']:.2f}")
    ...
    while True:
        ...
        elapsed_time = (current_time - last_candle['timestamp']) // 1000
        if elapsed_time >= 300:  # 5 minutes in seconds
            break
        print(f"Excutando estratégia... {elapsed_time} segundos", end="\r")
